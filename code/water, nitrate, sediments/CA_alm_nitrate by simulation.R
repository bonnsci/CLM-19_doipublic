...
ndat.site <- ndatyrw[ndatyrw$till %in% c("CT", "NT") & ndatyrw$cc %in% c("NC", "TC"),] %>%  # for stats
...
ndat_mean <- ndatyrw[ndatyrw$till %in% c("CT", "NT") & ndatyrw$cc %in% c("NC", "TC"),] %>%    # for plotting means
...
no3aov <- aov(NO3.sitemean ~ cc*nfert, data=ndat.site)  # & ndat.site$nfert=="Conventional N",])  #  tcc:nfert
...
# -25.740  -9.686   4.521   8.876  20.817 
# 
# Coefficients:
#   Estimate Std. Error t value Pr(>|t|)    
# (Intercept)          4.305e+01  2.993e+00  14.383  < 2e-16 ***
#   ccTC                -5.064e+00  4.233e+00  -1.196    0.236    
# nfertReduced N      -1.858e+01  4.233e+00  -4.389 4.68e-05 ***
#   ccTC:nfertReduced N -1.332e-14  5.986e+00   0.000    1.000    
# ---
#   Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
# 
# Residual standard error: 11.97 on 60 degrees of freedom
# Multiple R-squared:  0.4083,	Adjusted R-squared:  0.3787 
# F-statistic:  13.8 on 3 and 60 DF,  p-value: 5.968e-07
...
# cc           1    410     410   2.863   0.0958 .  
# nfert        1   5522    5522  38.531 5.52e-08 ***
#   cc:nfert     1      0       0   0.000   1.0000    
# Residuals   60   8599     143                     
# ---
#   Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
...
# <chr> <chr>                  <dbl>       <dbl> <chr>
#   1 NC    Conventional N          43.0       0.609 a    
# 2 TC    Conventional N          38.0       0.529 a    
# 3 NC    Reduced N               24.5       0.373 b    
# 4 TC    Reduced N               19.4       0.294 b 
# # LC conv N - Reduced N 
...
mean(c(0.49, 0.43))  # 0.46
mean(c(18.6, 18.5))  # 18.6
...
ggplot(data=ndat_mean, 
  ...
  geom_bar(stat="identity", position=position_dodge(), width=0.7, show.legend=F) + # color="#332288", 
  geom_errorbar(aes(ymin = NO3.mean.lbac - NO3.se.lbac, 
   ...
   facet_grid(# rows=vars(factor(till, levels=c("CT", "RT", "NT"))), 
     ...
     cols=vars(factor(cc, levels=c("NC", "TC"))), #,factor(till, levels=c("CT", "NT"))), 
             labeller = as_labeller(
               c(NC="No Cover Crop",TC="Triticale Cover"))) +
...
ggsave("plots/water, nitrate, sediments/CA_alm_NO3 losses mean annual bars by nfert and cc, lbac_no letters.png", width=5, height=4, dpi=300)
