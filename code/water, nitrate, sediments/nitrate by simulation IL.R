...
ndat_tmttot <- ndatyr %>%
  ...
  summarize(N2O.tot = sum(N2O.yr),  # this is like two totals cause each site-till-cc-nfert has a corn and a soy total  per year
            NO3.tot = sum(NO3.yr),
...
ndat_dec <- ndatyr %>%
  ...
  summarize(N2O.sitemean = mean(N2O.yr),  # mean annual N loss per site per decade
            NO3.sitemean = mean(NO3.yr)) %>%
  group_by(crop_system_name, till, cc, nfert, decade) %>%  # drop sitename to get means across sites
...
ggplot(data=ndeclong[ndeclong$variable=="NO3",], aes(x=nfert, y=mean, fill=decade)) +
