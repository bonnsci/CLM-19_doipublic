...
# stitch together
spei1dat <- bind_rows(spei1fit, .id="ID") %>%  # add list name as ID column, e.g., "NY-n_1.rcp26"
  separate_wider_delim(ID, delim=".", 
                       names=c("site", "rcp"),  # split the ID into two columns
                       cols_remove=T)

spei12dat <- bind_rows(spei12fit, .id="ID") %>%  # add list name as ID column, e.g., "NY-n_1.rcp26"
  separate_wider_delim(ID, delim=".", 
                       names=c("site", "rcp"),  # split the ID into two columns
                       cols_remove=T)
...
# write.csv(speidat, "data/climate data not large/speiNY.csv", row.names=F)
...
# climate - SPEI
spei <- read.csv("data/climate data not large/speiNY.csv")
...
write.csv(spei, "data/climate data not large/speiNY_formerge.csv", row.names = F)
...
ggsave("plots/climate/NY_SPEI_12-month.png", width=8, height=4, dpi=300)
  


################ SUMMER SPEI RCP 6.0 ONLY, 7 instead of 11 spei categories, AND all-NY together for AGU
...
ggsave("plots/climate/NY_SPEI11cats_seasonal averages_SUMMER ONLY.png", width=10, height=4, dpi=300)
...
ggsave("plots/climate/NY_SPEI7cats_seasonal averages_SUMMER ONLY.png", width=11, height=3, dpi=300)
