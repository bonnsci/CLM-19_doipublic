...
bmvin <- read.csv("data/biomass/biomass_vineyard.csv")
...
# 
# min(bmvin$Year) # 2013 --2013-2021 start up years, start at 2022
# max(bmvin$Year) # 2073

bmvin <- bmvin[bmvin$Year>2020,]

unique(bmvin$site_name) # 16 sites
unique(bmvin$region_name) # CA only
unique(bmvin$management_name) # "ct-nc" "nt-nc" "rt-nc" "cn"    "rn"    "ct-LegC" "rt-LegC" "ct-bc" "nt-bc" "nt-LegC" "rt-bc"
...
# -LegC = legume cover crop (Faba bean)       ---applies to alley
# -bc = basic (barley) cover crop        ---applies to alley
unique(bmvin$crop_system_name) # "grape-a" "grape-c"
# -a is the alley and -c is the crop

unique(bmvin$crop_name) # "fallow"           "grape"           "faba_bean"        "barley"

# make dummy factor for CT, RT, NT
bmvin$till <- ifelse(grepl("ct-", bmvin$management_name), "CT", 
                    ifelse(grepl("rt-", bmvin$management_name), "RT", 
                           ifelse(grepl("nt-", bmvin$management_name), "NT", "NA")))
# # check
# unique(bmvin$till)

# b <- bmvin[bmvin$management_name %in% c("ct-LegC", "rt-LegC", "nt-LegC"),]

# # check
# unique(bmvin$till)

# dummy for CC or NC
bmvin$cc <- ifelse(grepl("-nc", bmvin$management_name), "NC", 
                   ifelse(grepl("-bc", bmvin$management_name), "BarC", 
                          ifelse(grepl("-lc", bmvin$management_name), "LegC", "NA")))  #barley, legume
...
bmvin$nfert <- ifelse(grepl("cn", bmvin$management_name), "Conventional N", 
                     ifelse(grepl("rn", bmvin$management_name), "Reduced N", "NA"))

bmvin$fieldpart <- ifelse(grepl("-a", bmvin$crop_system_name), "alley", "crop")
# # check
# unique(bmvin$nfert)

# dummy for decade
bmvin$decade <- ifelse(bmvin$Year <2021, "2010s",
                      ifelse(bmvin$Year>=2021 & bmvin$Year <2031, "2020s",
                        ifelse(bmvin$Year>=2031 & bmvin$Year <2041, "2030s",
                          ifelse(bmvin$Year>=2041 & bmvin$Year <2051, "2040s",
                            ifelse(bmvin$Year>=2051 & bmvin$Year <2061, "2050s",
                              ifelse(bmvin$Year>=2061 & bmvin$Year <2071, "2060s", "2070s"))))))
# unique(bmvin$decade)
...
# 
# bmvin <- left_join(bmvin, precip.summer, 
#                    relationship = "many-to-one", 
#                    by = join_by(site_name, climate_scenario, Year==year))


# unique(bmvin$management_name)
...
ggplot(data=biomass_summary[biomass_summary$crop_name == "grape",],
...
ggsave("plots/biomass/CA_grape_biomass_bars.png", width=8, height=5, dpi=300)
...
bm_cover_sum <- bmvin %>%
  filter(crop_name %in% c("faba_bean", "barley")) %>%
...
ggsave("plots/biomass/CA_grape_coverbiomass_bars.png", width=8, height=5, dpi=300)
