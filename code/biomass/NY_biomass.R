...
unique(bm$management_name) # "ct-cc" "ct-nc" "nt-cc" "nt-nc" "rt-cc" "rt-nc"
...
# were simulated such that alfalfa, corn, and triticale were grown across each 
# calendar year (i.e. 
# simulation v1 - alfalfa is the first crop planted, "-act" 
#     years 2013-2016 in alfalfa, 2017 corn, 2017/2018, triticale (planted 11/2017, harvested 7/2018)
# simulation v2 - corn is the first crop planted, "-cta"
#     year 2013 is corn, 2013/2014 triticale, 2014-2017 alfalfa
# simulation v3 - triticale is the first crop planted) "-tac"
#     year 2013/14 is triticale, 2014-2017 alfalfa, 2018 corn
...
# dummy for N treatment # no N treatments in NY
...
bm_bu_decsum <- group_by(bm_bu, climate_scenario,crop_name, cc, till, decade) %>%
...
bm_tn_decsum <- group_by(bm_tn, climate_scenario,crop_name, cc, till, decade) %>%
...
ggplot(data=bm_bu_decsum[bm_bu_decsum$climate_scenario == "rcp60" ,], 
       aes(x=crop_name, y=grain.buac_mean, fill=decade)) +
...
ggplot(data=bm_tn_decsum[bm_tn_decsum$climate_scenario == "rcp60" ,], 
       aes(x=crop_name, y=bm.tac_mean, fill=decade)) +
  geom_bar(stat="identity", position=position_dodge(), color="#332288") +
